# Government colour standards

The following colour palette is recommended for standard data science visualisations and has been incorporated into the powerBI standard temple json.

## Bar Chart (colour palette)

<center>

![](pictures/bar_chart_colour_palette.png "The bar chart colour palette."){width=75%}

</center>

1st = #1D70B8, 2nd = #BC65BD, 3rd = #F05CA1, 4th = #FF6F61, 5th = #FFA600

## Bar Chart (single palette)

<center>

![](pictures/bar_chart_single_palette.png "The bar chart single palette."){width=75%}

</center>

1st = #1D70B8

## Bar Chart (tinted palette)

<center>

![](pictures/bar_chart_tinted_palette.png "The bar chart tinted palette."){width=75%}

</center>

1st = #1D70B8, 2nd = #3088C8, 3rd = #489FD6, 4th = #62B7E4, 5th = #7FCFF2

## Combi Chart (colour palette)

<center>

![](pictures/combi_chart_colour_palette.png "The combi chart colour palette."){width=75%}

</center>

1st = #1D70B8, 2nd = #F05CA1, 3rd = #FFA600

## Combi Chart (tinted palette)

<center>

![](pictures/combi_chart_tinted_palette.png "The combi chart tinted palette."){width=75%}

</center>

1st= #1D70B8, 2nd = #489FD6, 3rd = #FFA600

## Line Chart (colour palette)

<center>

![](pictures/line_chart_colour_palette.png "The line chart colour palette."){width=75%}

</center>

1st = #1D70B8, 2nd = #BC65BD, 3rd = #F05CA1, 4th = #FF6F61, 5th = #FFA600

## Line Chart (tinted palette)

<center>

![](pictures/line_chart_tinted_palette.png "The line chart tinted palette."){width=75%}

</center>

1st= #1D70B8, 2nd = #3088C8, 3rd = #489FD6, 4th = #62B7E4, 5th = #7FCFF2

## Additional resources: 

https://style.ons.gov.uk/category/data-visualisation/

https://design-system.service.gov.uk/styles/colour/
